...
tx_gain = -20
phase_offset = 0  # calibration between the two Rx channels in radians
...
choice = input("Plot [1] Conventional only or [2] Conventional and MVDR? [1/2]: ").strip()
show_mvdr = (choice == "2")
window_open = True
def _on_close(event):
    global window_open
...
print("Starting, use control-c or close the plot window to stop")
try:
    while True:
        if not window_open: # plot window was closed
            raise KeyboardInterrupt
        data = my_sdr.rx() # receive a batch of samples, it's a list of lists
        x = np.array(data) # shape is 2 x rx_buffer_size
        x[1] *= np.exp(-1j * phase_offset) # remove Rx1-Rx2 phase calibration offset
        ...
        if show_mvdr:
            ...
            plt.legend(['Conventional'], loc='lower left', fontsize='8')
